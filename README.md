...
canvas. Every pointer movement, shape edit, and AI-generated change is
propagated to other collaborators in well under a second. The AI agent adds a
...
- Sub-100 ms broadcast latency for object mutations handled by the
  `useRealtimeCanvas` hook, with reconnection queues that replay missed
  operations once connectivity returns.
- Live cursor broadcasting and presence cleanup handled through the
  ...
  other collaborators.
- Persistent Supabase sessions with safe teardown on browser unload via
  `/api/logout` and session utilities.
...
- Queue-backed processing ensures AI actions stay ordered, visible to all
  clients, and auditable through Supabase (`ai_operations_queue`).
...
- Connection status indicator, AI queue progress, and toasts provide feedback on
  background work and reconnection attempts.
...
- Each AI response lists the operations executed and updates the shared queue so
  other collaborators see progress indicators.
- Queue items may be inspected and retried from the chat transcript. Failed tool
  validations are surfaced back to the user for clarification.

Detailed contract and tool definitions live in
...
- **AI queue stuck in “processing”** – Verify the service-role key is available
  to the API route and that the table exists. Check
  `ai_operations_queue` in the Supabase dashboard for errors.
